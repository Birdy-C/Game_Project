...
public class TankMovement1 : MonoBehaviour {
    //设置坦克移动的速度
    public float tank_MoveSpeed = 10f;
    //设置坦克转向的速度
    ...
    {
        Vector3 movement;
		position_before = tank_Rigidbody.position.z;
        if (Input.GetKey(KeyCode.W) | Input.GetKey(KeyCode.UpArrow))
        {
			...
			if (move < 0f) 
			{
                ...
                if (tank_nowSpeed>0f)
				{
                    ...
                    movement = -transform.forward  * (tank_nowSpeed * Time.deltaTime - 1 / 2 * tank_acc * Time.deltaTime * Time.deltaTime);
					tank_nowSpeed = tank_nowSpeed - tank_acc * Time.deltaTime;
					tank_Rigidbody.MovePosition (tank_Rigidbody.position + movement);
					position_now = tank_Rigidbody.position.z;
				} 
			}
				
			else if (tank_nowSpeed < tank_MoveSpeed) 
			{
                Debug.Log("C");
                movement = transform.forward * tank_MoveInputValue * (tank_nowSpeed * Time.deltaTime + 1 / 2 * tank_acc * Time.deltaTime * Time.deltaTime);
				tank_nowSpeed = tank_nowSpeed + tank_acc * Time.deltaTime;
				tank_Rigidbody.MovePosition (tank_Rigidbody.position + movement);
				position_now = tank_Rigidbody.position.z;
			} 
		...
		move = position_before - position_now;
        Debug.Log(move);
